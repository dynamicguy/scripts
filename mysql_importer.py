...
def process_row(row):
    ...
    if uri != None:
        fullImageUrl = 'https://assetsds.cdnedge.bluemix.net/sites/default/files/' + uri.replace('public://', '')
    else:
        fullImageUrl = None
        ...
        if result.with_rows:
            ...
            for row in rows:
                ...
                if news_response.status_code == 200:
                    ...
                    print('%s news found' % len(news_found))
                    if len(news_found) < 1:
                        if news.get('featuredImage') != None:
                            img_hash = upload_img(news.get('featuredImage'), news.get('id'))
                        else:
                            img_hash = 'fcbf712ca769cb7f704e8a162055384d'

                        print('image hash was: %s' % img_hash)
                                ...
                                for cat_row in cat_rows:
                                    ...
                                    if cat_response.status_code == 200:
                                        ...
                                        if len(cats) < 1:
                                            ...
                                            cat_create_response = create_category(category)
                                            if cat_create_response.status_code == 201:
                                                news_cat = cat_create_response.json()

                                        else:
                                            news_cat = cats[0]

                                        print('adding category %s' % news_cat)
                                        news.get('categories', []).append(news_cat)
